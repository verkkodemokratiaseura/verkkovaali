...
def _setup_delivery():
    ...
    hostname = settings.get('webidentity_smtp_host', 'localhost').strip()
    try:
        port = int(settings.get('webidentity_smtp_port', '25').strip())
    except (ValueError, TypeError):
        port = 25
    debug = settings.get('webidentity_smtp_debug', 'false').strip().lower() == 'true'
